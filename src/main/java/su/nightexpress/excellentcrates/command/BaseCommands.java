...
Lang.COMMAND_DROP_DONE.message().send(context.getSender(), replacer -> replacer
    ...
    private boolean giveCrate(@NotNull CommandContext context, @NotNull ParsedArguments arguments) {
        ...
        this.plugin.getUserManager().manageUser(targetName, user -> {
            if (user == null) {
                // Fallback: if user is not known on this node, try cross-server by name via Redis.
                boolean dispatched = this.plugin.getRedisSyncManager()
                    .map(sync -> {
                        this.plugin.getCrateManager().giveCrateItemCrossServerByName(crate, targetName, amount);
                        return true;
                    }).orElse(false);
                if (!dispatched) {
            ...
            if (online != null) {
                    this.plugin.getCrateManager().giveCrateItem(online, crate, amount);
                ...
                this.plugin.getCrateManager().giveCrateItemCrossServer(crate, user.getId(), amount);
            }
